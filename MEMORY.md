...
### UI (feature/ui-overhaul, PR #19)
- **Branding**: NLM (was LLMRank); Playfair Display serif headings; gold `#C9A644` accent; `#0c0e14` dark bg
...
- **Your Business**: collapsed by default; services/signals expand on demand (4/5 shown initially)
- **AI Visibility Score**: two-column layout (scores left, "What this means" right)
- **Recommended Improvements**: combined recs + actions; flipable carousel (front = problem, back = action plan + copy); navigation dots; 88%/12% peek at next card
- **Flip card CSS**: in `globals.css` — `.flip-container`, `.flip-card-inner.is-flipped`, `.flip-card-face`, `.flip-card-back`
